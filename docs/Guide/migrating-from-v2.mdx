...
In v3 we have added many changes and performance updates which help your code look and feel better.
But before we hop in, let's see the breaking changes.

## Breaking changes
In v3 we have removed all methods such as `getBrawlers` and `getRotation`, etc from the client so as a replacement we have adopted managers system, so here is how your code should change:
