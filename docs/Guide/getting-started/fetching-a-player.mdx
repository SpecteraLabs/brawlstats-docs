...
```javascript
client.players.fetch('player-tag')
  .then(player => {
	console.log(player);
  })
  .catch(error => {
...
Instead of using `player-tag` as your tag, you can also use `#player-tag` or `%23player-tag` to get the same result. 
For example, `22QJ0JPVJ`, `#22QJ0JPVJ` and `%2322QJ0JPVJ` all return same player. 
...
```javascript
client.players.fetchMany('player-tag-1', 'player-tag-2', 'player-tag-3')
  .then(player => {
	console.log(player);
  })
  .catch(error => {
